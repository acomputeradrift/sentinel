...
def create_test_apex(
    ...
    *,
    ...
    # Add raw graphics-id columns after fixture inserts so legacy positional inserts remain valid.
    cur.execute("alter table RTIDeviceButtonData add column UpBitmapId integer")
...
class ScriptContractsTest(unittest.TestCase):
    def test_extract_creates_project_data_json(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertFalse(image6["testTargets"]["graphics"]["bitmap"])
            self.assertFalse(image6["testTargets"]["graphics"]["icon"])
            self.assertEqual(image14["buttonIdentity"]["buttonType"], "Image")
            self.assertTrue(image14["testTargets"]["variables"]["Image"])
            self.assertFalse(image14["testTargets"]["graphics"]["bitmap"])
            self.assertTrue(image14["testTargets"]["graphics"]["icon"])
