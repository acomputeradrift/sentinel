...
class TestingResultPostingTest(unittest.TestCase):
    ...
    def _wait_for_button_count(self, page, selector: str, text: str, *, timeout_s: float = 3.0) -> None:  # noqa: ANN001
        deadline = time.time() + timeout_s
        while time.time() < deadline:
            try:
                got = page.locator(selector).first.locator(".test-btn-count").inner_text()
            except Exception:
                got = ""
            if (got or "").strip() == text:
                return
            time.sleep(0.05)
        self.fail(f"Expected {selector} .test-btn-count to be {text!r}")

    def _wait_for_button_state_class(self, page, selector: str, class_name: str, *, timeout_s: float = 3.0) -> None:  # noqa: ANN001
        deadline = time.time() + timeout_s
        while time.time() < deadline:
            try:
                ok = bool(page.locator(selector).first.evaluate("(el, cls) => el.classList.contains(cls)", class_name))
            except Exception:
                ok = False
            if ok:
                return
            time.sleep(0.05)
        self.fail(f"Expected {selector} to have class {class_name!r}")
...
""",
    ...
    def test_snapshot_rehydrates_pass_and_fail_after_reload(self):
        ...
        try:
            ...
            page.click(".btn-wrap .test-btn")
            self._wait_for_row_status_contains(page, 0, "PASS")
            self._wait_for_row_status_contains(page, 0, "2026-03-25T10:00:00Z")
            self._wait_for_row_status_contains(page, 1, "FAIL")
            self._wait_for_row_status_contains(page, 1, "2026-03-25T10:00:01Z")
        finally:
            server.stop()

    def test_button_visual_state_and_counts_update_without_behavior_changes(self):
        from sentinel.generation.render_core import render_single_device_html, load_json
                                            ...
                                            "screenButtons": [
                                                {
                                                    ...
                                                    "testTargets": {"text": True, "macros": False, "macroSteps": False, "variables": {}, "pageLink": {"enabled": True, "targetPageId": 514}},
                                                    "resolvedPageLink": {"targetPageId": 514},
                                                },
                                                {
                                                    "buttonIdentity": {"buttonTagName": "BTN-2", "text": "No Targets", "buttonType": None},
                                                    "buttonUI": {
                                                        "fontSize": 10,
                                                        "orientations": {"portrait": {"visible": True, "coordinates": {"top": 64, "left": 10, "height": 44, "width": 120}}},
                                                    },
                        ...
                        "pages": [
                            {
                                ...
                                "buttons": [
                                    {"buttonId": 48551, "buttonTagName": "BTN-1", "identifiers": {"text": "Button 1"}, "testTargets": {}},
                                    {"buttonId": 48552, "buttonTagName": "BTN-2", "identifiers": {"text": "No Targets"}, "testTargets": {}},
                                ],
                                "viewports": [],
                            }
                        ],
        ...
        try:
            ...
            self._wait_for_button_state_class(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "fill-screen-control")
            self._wait_for_button_state_class(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "state-untested")
            self._wait_for_button_count(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "0/2")
            self._wait_for_button_count(page, ".btn-wrap[data-diag-button-id='48552'] .test-btn", "0/0")
...
() => window.__emitWs({
  ...
  projectId: "proj-1",
  recordedAtUtc: "2026-03-21T00:00:00Z",
  targetKey: "btn:81:513:48551:Text",
  outcome: "FAIL"
})
"""
            )
            self._wait_for_button_state_class(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "state-fail")
            self._wait_for_button_count(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "0/2")
...
"""
            ...
            self._wait_for_button_state_class(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "state-partial")
            self._wait_for_button_count(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "1/2")
...
() => window.__emitWs({
  type: "test_result",
  projectId: "proj-1",
  recordedAtUtc: "2026-03-21T00:00:02Z",
  targetKey: "btn:81:513:48551:PageLink",
  outcome: "PASS"
})
"""
            )
            self._wait_for_button_state_class(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "state-pass")
            self._wait_for_button_count(page, ".btn-wrap[data-diag-button-id='48551'] .test-btn", "2/2")

            # Existing interaction path still works.
            page.click(".btn-wrap[data-diag-button-id='48551'] .test-btn")
