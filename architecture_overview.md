...
## System Summary
1. Sentinel follows a staged pipeline: uploaded `.apex` file -> project-specific JSON shaped by `project_structure.json` -> generated event testing and device testing interfaces shaped by `app_ui_structure.json`.
...
2. `project_structure.json` is the controlling contract for generated project-specific JSON and must not be bypassed by ad hoc extraction behavior.
