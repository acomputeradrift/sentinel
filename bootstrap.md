...
- **Constraints:** `DATABASE_URL` absent ⇒ `InMemoryRepository` (tests/local); present ⇒ Postgres + migrations on `PostgresRepository` init. Deploy is **commit + push**, then **pull that commit onto `/opt/sentinel/app`** (`docs/directives/dev_environment_and_workflow.md`). Migrations: **no dollar-quoted SQL** in `migrations/*.sql` unless splitter extended. Python **≥3.11** declared; CI uses **3.12**. UNC/SMB workspace: agent shells may need `SENTINEL_REPO_ROOT` / `SENTINEL_VENV_PYTHON` (`repo_paths.py`).
