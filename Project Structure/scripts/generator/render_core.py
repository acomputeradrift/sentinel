...
def render_html(project_data: dict[str, Any], app_ui: dict[str, Any], device_index: int = 0, page_index: int = 0) -> str:
    ...
    for btn, label, off_top, off_left in _iter_page_buttons(page):
        ...
        meta = {
            "category": label,
            "identity": _btn_text(btn.get("buttonIdentity", {})),
            "buttonType": btn.get("buttonIdentity", {}).get("buttonType") or "",
            "targets": _targets(btn, variable_label),
        }
        page_button_rows.append(
            "<button class='btn' style='left:{left}px;top:{top}px;width:{width}px;height:{height}px;font-size:{fs}px' data-meta='{meta}'>{text}</button>".format(
                left=int(c.get("left") or 0) + off_left,
                top=int(c.get("top") or 0) + off_top,
                width=int(c.get("width") or 0),
                height=int(c.get("height") or 0),
                fs=fs,
                meta=json.dumps(meta).replace("'", "&apos;"),
                text=_btn_text(btn.get("buttonIdentity", {})),
            )
        )
    ...
    for vb in _iter_viewport_buttons(page):
        ...
        viewport_button_rows.append(
            "<button class='btn vp-btn' data-vp='{vp}' data-frame='{frame}' style='left:{left}px;top:{top}px;width:{width}px;height:{height}px;font-size:{fs}px;{extra}' data-meta='{meta}'>{text}</button>".format(
                vp=vb["vp_index"],
                frame=vb["frame_id"],
                left=int(c.get("left") or 0) + int(vb["off_left"]),
                top=int(c.get("top") or 0) + int(vb["off_top"]),
                width=int(c.get("width") or 0),
                height=int(c.get("height") or 0),
                fs=fs,
                extra=extra,
...
.btn{{position:absolute;box-sizing:border-box;border:0;border-radius:10px;background:#1e5f86;box-shadow:inset 0 0 0 1px #154665;color:#fff;line-height:1.1;white-space:pre-line;cursor:pointer;overflow:hidden;padding:0;z-index:2;}}
